<script lang="ts">
	...
	import MarkmapRenderer from './MarkmapRenderer.svelte';
	...
	let _code = '';
	let _lastCodeFromProps = '';

	// Keep editor state in sync with incoming `code` prop without clobbering unsaved edits.
	$: if (code !== _lastCodeFromProps) {
		// Only overwrite editor content if the user hasn't changed it since the last sync.
		if (_code === _lastCodeFromProps) {
			_code = code;
		}
		_lastCodeFromProps = code;
	}
	...
	let markmapMode: 'code' | 'render' = 'code';

	// 让 TS/编辑器有类型（否则有的环境会报“未声明变量”）
	let isMarkmap = false;
	let normLang = '';
	let looksLikeMarkmap = false;
	let displayLang = '';

	const normalizeLang = (s = '') => (s || '').trim().toLowerCase();
	$: normLang = normalizeLang(lang);

	// 兜底：如果 fence 写的是 markdown/空，但内容包含 markmap frontmatter，也当作 markmap
	$: looksLikeMarkmap =
		(normLang === 'markdown' || normLang === '') && /^\s*---[\s\S]*?\n\s*markmap\s*:/i.test(_code);

	$: isMarkmap = ['markmap', 'mm'].includes(normLang) || looksLikeMarkmap;

	// ✅ UI 展示：只要识别为 markmap，就显示“markmap”，不显示“markdown”
	$: displayLang = isMarkmap ? 'markmap' : lang || '';

	// 防止切到别的语言时还停留在 render
	$: if (!isMarkmap) {
		markmapMode = 'code';
	}
	...
	const previewCode = () => {
		// Always preview the latest editor value
		onPreview(_code);
	};
	...
	const render = async () => {
		...
		// Use normalized language for robustness (e.g., 'Mermaid' / 'MERMAID').
		if (normLang === 'mermaid' && (token?.raw ?? '').slice(-4).includes('```')) {
			try {
				renderHTML = await renderMermaid(_code);
			} catch (error) {
			...
			(normLang === 'vega' || normLang === 'vega-lite') &&
			(token?.raw ?? '').slice(-4).includes('```')
		) {
			try {
				renderHTML = await renderVegaVisualization(_code);
			} catch (error) {
...
<div>
	<div
		...
		dir="ltr"
	>
		{#if ['mermaid', 'vega', 'vega-lite'].includes(normLang)}
			{#if renderHTML}
		...
		{:else}
			...
			>
				<div class="flex items-center gap-0.5">
					...
					{#if ($config?.features?.enable_code_execution ?? true) && (normLang === 'python' || normLang === 'py' || (normLang === '' && checkPythonCode(_code)))}
						{#if executing}
					...
					{#if isMarkmap}
						<button
							class="flex gap-1 items-center run-code-button bg-none border-none transition rounded-md px-1.5 py-0.5 bg-white dark:bg-black"
							on:click={() => {
								markmapMode = markmapMode === 'code' ? 'render' : 'code';
							}}
						>
							<div>{markmapMode === 'code' ? '渲染' : '代码'}</div>
						</button>
					{/if}

					{#if preview && ['html', 'svg'].includes(normLang)}
						<button
							class="flex gap-1 items-center run-code-button bg-none border-none transition rounded-md px-1.5 py-0.5 bg-white dark:bg-black"
			...
			>
				...
				{#if !collapsed}
					{#if isMarkmap && markmapMode === 'render'}
						<!-- 用 _code：支持“没点保存也能渲染最新编辑内容” -->
						<div class="p-3 bg-white dark:bg-black">
							<MarkmapRenderer markdown={_code} />
						</div>
					{:else}
						{#if edit}
							<CodeEditor
						...
						{:else}
							<pre
								...
								style="border-top-left-radius: 0px; border-top-right-radius: 0px; {(executing ||
									...
									>{@html hljs.highlightAuto(_code, hljs.getLanguage(lang)?.aliases).value || _code}</code
								></pre>
						{/if}
					{/if}
				{:else}
			...
			{#if !collapsed && !(isMarkmap && markmapMode === 'render')}
				<div
					id="plt-canvas-{id}"
