...
# ===== HARD DEFAULT: max_tokens =====
DEFAULT_MAX_TOKENS = 8192  # <- 你要写死的默认值

def _ensure_default_max_tokens(payload: dict) -> None:
    """
    If request doesn't specify max_tokens/max_completion_tokens,
    force a hard default to avoid short/truncated outputs.
    """
    if not isinstance(payload, dict):
        return

    mt = payload.get("max_tokens", None)
    mct = payload.get("max_completion_tokens", None)

    # treat missing/None/0/"" as "not provided"
    def _missing(x):
        return x is None or x == "" or x == 0

    if _missing(mt) and _missing(mct):
